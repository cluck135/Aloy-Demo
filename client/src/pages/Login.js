...
const Login = () => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      setErrorMessages({ message: "Username or password was incorrect" });
      console.error(e);
    }
  }
    ...
    <div className='p-4 flex bg-MHgreen h-screen lg:justify-center'>
      <div className='m-4 h-fit bg-white rounded-lg shadow-lg lg:w-1/4'>
        <form className="flex flex-col p-5" onSubmit={handleFormSubmit}>
          <h1 className="pb-2 text-2xl">Login to Loyalty Rewards</h1>
        ...
        <div className="flex justify-center mt-5">
          <Link to="/signup">
            <div className="p-2 m-4 w-fit rounded-md bg-sky-300 text-xs">Signup here</div>
          </Link>
        </div>
      </div>
    </div>
